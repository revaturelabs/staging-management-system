...
public class DataGeneration {
    ...
    class AssociateP extends Associate {
        ...
        AssociateP(Associate a) {
            ...
            else clientProbabilityMultiplier = 1; //The average associate corresponds with the client probabilities.
    ...
    public void generate() {
        ...
        for (Client c : allClients) {
            if (c.isPriority()) priorityClients.add(new ClientP(c));
            else regularClients.add(new ClientP(c));
        }
        for (Associate a : associates) {
            ...
            while (confirmDate == null && currDate.compareTo(LocalDateTime.now()) < 0) {
                ...
                if (interview) {
                    ...
                    if (currDate.compareTo(LocalDateTime.now()) <= 0) is = client.evaluateAssociate(ap);
                    else is = interviewStatusService.findByStatus("MAPPED");
            ...
            if (jobStartDate == null) jobStartDate = currDate;
    ...
    private void createCheckins(LocalDateTime batchEndDate, LocalDateTime startDate, Associate associate) {
        ...
        while (currDate.compareTo(startDate) <= 0 && currDate.compareTo(LocalDateTime.now()) <= 0) {
            ...
            if (day != DayOfWeek.SUNDAY && day != DayOfWeek.SATURDAY) {
                Checkin checkin = new Checkin(0l, currDate.withHour(9), currDate.withHour(17), null, null, associate);
    ...
    private int logRythmicConvergence(int start, int end, double convergenceFactor) {
        ...
        while (totalProbability < 1) {
            ...
            if (totalProbability * 100 > rollDice) return tval % end;
