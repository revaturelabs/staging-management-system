...
BEGIN
    RET_VAL.EXTEND();
    SELECT STAGGING_ASSOC_TYPE(ID, ASSOCIATE_NAME, ASSOCIATE_PORTFOLIO_LINK, checkins.CHECKIN_IN_TIME) 
    ...
    FROM (SELECT ASSOCIATE_ID AS ID, ASSOCIATE_NAME, ASSOCIATE_PORTFOLIO_LINK FROM
            (SELECT ASSOCIATE_ID, ASSOCIATE_NAME, ASSOCIATE_PORTFOLIO_LINK, JOB_COUNT, COUNT(ASSOCIATE_ID) AS OCCURANCE_COUNT FROM 
                (SELECT * FROM ASSOCIATE_STATE_INFO
                    WHERE JOB_BUYOUT_DATE IS NULL AND 
                        ((JOB_START_DATE > TO_TIMESTAMP (V_DATE, 'DD-Mon-RR HH:MI:SS.FF AM') OR JOB_ACTUAL_END_DATE < TO_TIMESTAMP (V_DATE, 'DD-Mon-RR HH:MI:SS.FF AM') OR JOB_START_DATE IS NULL) AND 
                        (BATCH_END_DATE < TO_TIMESTAMP (V_DATE, 'DD-Mon-RR HH:MI:SS.FF AM'))))
    ...
    LEFT join checkins on ID=CHECKINS.ASSOCIATE_ID WHERE TO_CHAR(checkins.CHECKIN_IN_TIME, 'MM-DD-YY')=TO_CHAR(TO_TIMESTAMP (V_DATE, 'DD-Mon-RR HH:MI:SS.FF AM'), 'MM-DD-YY') OR CHECKINS.CHECKIN_IN_TIME IS NULL;
