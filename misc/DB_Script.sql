...
create or replace FUNCTION GET_ATTENDANCE_REPORT (V_DATE VARCHAR2)
  RETURN ATTENDANCE_TABLE
IS
  RET_VAL ATTENDANCE_TABLE := ATTENDANCE_TABLE();
BEGIN
    RET_VAL.EXTEND();
    SELECT ATTENDANCE_TYPE(CHECKED_IN, NOT_CHECKED_IN, TO_TIMESTAMP(V_DATE)) 
    INTO RET_VAL FROM
        (SELECT 
            (SELECT Count(*) FROM TABLE(GET_STAGGING_ASSOC_BY_DATE('01-JUN-17')) where checkin_date_time is not null) AS CHECKED_IN,
            (SELECT Count(*) FROM TABLE(GET_STAGGING_ASSOC_BY_DATE('01-JUN-17')) where checkin_date_time is null) as NOT_CHECKED_IN
        FROM DUAL);
